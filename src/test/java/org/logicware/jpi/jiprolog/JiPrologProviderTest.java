...
public class JiPrologProviderTest extends JiPrologBaseTest {
	...
	// iPrologEngine.find("employee(Name,Dpto,Scale),department(Dpto,DepartmentName),salary(Scale,Money),Money
	// < 2000");
	// Map<String, IPrologTerm> map =
	// iPrologEngine.find(adapter.parseTerms("employee(Name,Dpto,Scale),department(Dpto,DepartmentName),salary(Scale,Money),Money
	// < 2000"));
	...
	JIPFunctor eValue = JIPFunctor.create("<",
		JIPCons.create(JIPVariable.create("HJ"), JIPCons.create(JIPNumber.create(200), null)));
