...
final class GlobalDB extends Object// implements Cloneable //Serializable
{
	...
	// Database
	Hashtable<String, JIPClausesDatabase> m_clauseTable;
	...
	public final boolean isSystem(final String strName) {
		return m_clauseTable.containsKey(SYSTEM_MODULE + ":" + strName) || m_clauseTable.containsKey(KERNEL_MODULE + ":" + strName) || BuiltInFactory.isBuiltIn(strName) || strName.equals(",/2");
	}
	...
	final void abolish(PrologObject pred) {
		...
		try {
			while (pred != null) {
				...
				// m_clauseTable.remove(strModuleName + ":" +
				// ((Functor)head).getName());
	...
	private final synchronized void addPredicate(final Clause clause, final boolean bFirst, final String strFile, boolean dynamic) {
		...
		if (m_clauseTable.containsKey(strFunctName)) {
			...
			if (dynamic && !db.isDynamic()) {
				throw new JIPPermissionException("modify", "static_procedure", ((Functor) clause.getHead()).getName(), jipEngine);
				// throw JIPRuntimeException.create(30,
				// ((Functor)clause.getHead()).getName());
			}
	...
	final synchronized JIPClausesDatabase search(final Functor funct, final String strModule) {
		// System.out.println("SearchFor: " + strModule + ":" +
		// funct.getName()); // DBG
	...
	// if(strPredDef.startsWith(SYSTEM_MODULE) ||
	// strPredDef.startsWith(USER_MODULE))
	// {
		...
		for (String strPredName : predVector) {
			if (m_clauseTable.containsKey(strPredName)) {
				if (isMultifile(strPredName)) {
					JIPClausesDatabase db = m_clauseTable.get(strPredName);
