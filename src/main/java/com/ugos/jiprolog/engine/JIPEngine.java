 ...
 * See the section <i>"How to call JIProlog interpreter from Java"</i> in the
 * Reference Manual.
 * 
 * @version 4.0.5.x
...
public class JIPEngine implements Serializable {
	...
	/**
	 * Constructs a new instance of JIPEngine with default values search path,
	 * input and output stream. Search path is the path where this instance of
	 * JIPEngine searches for files. The default value is the value of the
	 * environment variable user.dir<br>
	 * Input and Output stream are the streams where this instance of JIPEngine
	 * reads and writes. The default values are respectively System.in and
	 * System.out.<br>
	 */
	public JIPEngine() {
	...
	/**
	 * Sets the update semantics ISO Prolog specifies that a prolog system
	 * should implement 'logical update semantics' on the internal database. The
	 * ISO standard imposes a static state of the database at and during the
	 * time of a query/goal. By 'static' it means that new clauses/entries to
	 * the database are not visible in back-tracking. The following example
	 * illustrates the difference between the two semantics:
	 * 
	 * :- dynamic a/1. JIP:- assert(a(1)). JIP:- retract(a(X)), X1 is X + 1,
	 * assertz(a(X)).
	 * 
	 * With 'immediate update semantics', the first calls to retract/1 and
	 * assertz/1 will succeed. On back-tracking, the system will retry retract/1
	 * and the newly asserted goal, which is visible to retract/1, can be
	 * retracted from the database. (This example will continue generating
	 * integers forever.) On back-tracking with 'logical update semantics' the
	 * call to retract/1 will not see the update performed by the assertz/1 and
	 * the query will have a single solution.
	 * 
	 * @param immediate
	 *            true - immediate update semantics, false - logical update
	 *            semantics (default)
	 */
	 ...
	 * 
	 * @return true if immediate update semantics, false if logical update
	 *         semantics (default)
	 */
	 ...
	 * The library should be a valid .jar file containing a set of extension
	 * classes.<br>
	 * If in the .jar there is a file named init.pl it is consulted as any other
	 * prolog file. For more information see the section
	 * <i>"Load an extension library"</i> in the Reference Manual.<br>
	 */
	public final void loadLibrary(final String strPath) throws IOException {
		...
		// final String strBasePath = (libf.getParent() != null ?
		// libf.getParent() : File.separator);
	 ...
	 * 
	 * @param listener
	 *            JIPEventListener object to add
	 ...
	 * 
	 * @param listener
	 *            JIPEventListener object to remove
	 * @see com.ugos.jiprolog.engine.JIPEventListener
	 */
	 ...
	 * 
	 * @param traceListener
	 *            JIPTraceListener object to add
	 ...
	 * 
	 * @param traceListener
	 *            JIPTraceListener object to remove
	 * @see com.ugos.jiprolog.engine.JIPTraceListener
	 */
	...
	/**
	 * Sets the search path of this instance of JIPEngine. The Search path is
	 * the path where this instance of JIPEngine searches for files.<br>
	 * It can refer to:<br>
	...
	/**
	 * returns the search path of this instance of JIPEngine. The Search path is
	 * the path where this instance of JIPEngine searches for files.<br>
	 * 
	...
	/**
	 * Sets the trace flag. If the flag is set all JIPTraceListeners are
	 ...
	 * 
	 * @param outs
	 *            the current output stream to set
	 * @see com.ugos.jiprolog.engine.JIPEngine#getUserOutputStream
	 */
	 ...
	 * 
	 * @param ins
	 *            the current input stream to set
	 * @param strStreamName
	...
	/**
	 * Closes all pending queries, reset the prolog interpreter and clear the
	 * database
	 */
	public void reset() {
		closeAllQueries();
	...
	/**
	 * Consults a file. If in the file there are some no multifile predicates
	 * already consulted/loaded in another file file/stream the interpreter
	 * raises an exception.<br>
	 * 
	 * @param strFileName
	 *            the name of the file to consult. It can refer to: <br>
	 *            - a file on hard disk (d:\myapp/myfile.pl,
	 *            file:/user/myapp/myfile.pl)<br>
	 *            - an URL (http://www.mysite.com/myfile.pl)<br>
	 *            - a file contained in a jar file
	 *            (jar://user/myprologfile.jar#myfile.pl)<br>
	...
	/**
	 * Consults a Stream. If in the stream there are some no multifile
	 * predicates already consulted/loaded in another file/stream the
	 * interpreter raises an exception.<br>
	 * 
	 * @param ins
	 *            is the stream to consult
	 * @param strStreamName
	 *            is the symbolic name of the stream
	 * @exception com.ugos.jiprolog.engine.JIPSyntaxErrorException
	 *                , java.io.IOException
	...
	/**
	 * Unconsults/unloads a file. Deletes from the prolog database all clauses
	 * previously consulted or loaded with to the specified file. Note: If in
	 * the file consulted/loaded there was nested consult/load the predicates
	 * defined in the other files are not deleted.
	 * 
	 * @param strFileName
	 *            the name of the file to unconsult.
	 * @see com.ugos.jiprolog.engine.JIPEngine#loadFile
	...
	/**
	 * Unconsults/unloads a stream. Deletes from the prolog database all clauses
	 * previously consulted or loaded with to the specified stream. Note: If in
	 * the file consulted/loaded there was nested consult/load the predicates
	 * defined in the other files are not deleted.
	 * 
	 * @param strStreamName
	 *            the name of the stream to unconsult.
	 * @see com.ugos.jiprolog.engine.JIPEngine#loadStream
	...
	/**
	 * Compile the specified file. As result a file with .cpl extension is
	 * produced<br>
	 * 
	 * @param strFileName
	 *            the name of the file to compile. It can refer to: <br>
	 *            - a file on hard disk (d:\myapp/myfile.pl,
	 *            file:/user/myapp/myfile.pl)<br>
	 *            - a path relative to the search path.
	 * @exception com.ugos.jiprolog.engine.JIPSyntaxErrorException
	...
	/**
	 * Loads a file. The file must contain a valid JIProlog compiled code If in
	 * the file there are some no multifile predicates already consulted/loaded
	 * in another file the interpreter raises an exception.<br>
	 * 
	 * @param strFileName
	 *            the name of the file to load. It can refer to: <br>
	 *            - a file on hard disk (d:\myapp/myfile.pl,
	 *            file:/user/myapp/myfile.pl)<br>
	 *            - an URL (http://www.mysite.com/myfile.pl)<br>
	 *            - a file contained in a jar file
	...
	/**
	 * Loads a stream. The stream must contain a valid JIProlog compiled code If
	 * in the stream there are some no multifile predicates already
	 * consulted/loaded in another file the interpreter raises an exception.<br>
	 * 
	 * @param ins
	 *            the inpustream to load
	 * @param strStreamName
	 *            the name of the file to load. It can refer to: <br>
	 *            - a file on hard disk (d:\myapp/myfile.pl,
	 *            file:/user/myapp/myfile.pl)<br>
	...
	/**
	 * It is equivalent to asserta/1 predicate. Add a clause to the interpreter
	 * database at the beginning of the clauses associated to the given one.
	 * 
	 * @param term
	 *            the term to assert.
	 */
	...
	/**
	 * It is equivalent to retract/1 predicate. Retracts a fact or a clause that
	 * unifies with the given term from the interpreter database and, as
	 * side-effect, unifies the given term the the one retracted.
	 * 
	 * @param term
	 *            the term to retract.
	 */
	...
	/**
	 * It is equivalent to assert/1 predicate. Add a term to the interpreter
	 * database
	 * 
	 * @param term
	 *            the term to assert.
	...
	/**
	 * It is equivalent to abolish/1 predicate. abolish a term in the
	...
	/**
	 * Opens a query. <br>
	 * The registered JIPEventListeners will be notified in the openNotified
	 * method.<br>
	 * When a solution is found the JIPEventListeners will be notified in
	 * solutionNotified.<br>
	 * If an error occurs the JIPEventListeners will be notified in
	 * errorNotified.<br>
	 * 
	 * @param strQuery
	 *            Query as a text string
	 ...
	 * The registered JIPEventListeners will be notified in the openNotified
	 * method.<br>
	 * When an event occurs in the interpreter the registered JIPEventListeners
	 * will be notified in the corresponding <i>notified</i> method.<br>
	 * If an error occurs the registered JIPEventListeners will be notified in
	 * errorNotified.<br>
	 * 
	 * @param jipquery
	 *            Query as a JIPTerm
	...
	public synchronized int openQuery(final JIPTerm jipquery) {
		...
		// crea una nuova WAM
		final WAM wam = m_bTrace ? new WAMTrace(this) : new WAM(this);
	...
	/**
	 * Searches for another soution. The registered JIPEventListeners will be
	 * notified in the openNotified method.<br>
	 * When an event occurs in the interpreter the registered JIPEventListeners
	 * will be notified in the corresponding <i>notified</i> method.<br>
	 * If an error occurs the registered JIPEventListeners will be notified in
	 * errorNotified.<br>
	 * If the query specified in the parameter is still running it raises a
	 * JIPIsRunningException If the query specified in the parameter is not
	 * found it raises a JIPInvalidHandleException
	 * 
	 * @param nQueryHandle
	...
	public synchronized void nextSolution(final int nQueryHandle) {
		...
		final AsyncWAMManager container = (AsyncWAMManager) m_prologTable.get(new Integer(nQueryHandle));
	 ...
	 * The registered JIPEventListeners will be notified in closeNotify. If the
	 * query specified in the parameter is not found it raises a
	 * JIPInvalidHandleException
	 * 
	 * @param nQueryHandle
	 *            the handle of the query to close
	 * @see com.ugos.jiprolog.engine.JIPEngine#nextSolution
	 * @see com.ugos.jiprolog.engine.JIPEngine#closeQuery
	 ...
	 * If the query specified in nQueryHandle is not found it raised a
	 * JIPInvalidHandleException.<br>
	 * If the query specified in nQueryHandle is still running it raises a
	 * JIPIsRunningException.<br>
	 * Note: You should not call isDeterministic until the query has no reached
	 * the goal.<br>
	 * 
	 * @param nQueryHandle
	 *            the handle of the query
	...
	public synchronized boolean hasMoreChoicePoints(final int nQueryHandle) {
		...
		// N.B. se la query è stata chiusa il container non compare nella
		// tabella
	 ...
	 * 
	 * @param nID
	 *            the ID of the event to notify
	 * @param term
	 *            the term attached to the event
	 * @param nQueryHandle
	 *            the handle of the query which notifies the event
	...
	// public final Enumeration<JIPClausesDatabase> getClausesDatabase() {
	// return m_globalDB.databases();
	// }
	...
	final void notifyException(final Throwable err, final int nQueryHandle) {
		...
		// {
		// notifyUndefPredicate(((UndefinedPredicateException)err).getTerm().getTerm(),
		// nQueryHandle);
		// }
		// else
	...
	// private final void notifyUndefPredicate(final PrologObject term, final
	// int nQueryHandle)
	// {
	// m_eventNotifier.notifyEvent(JIPEvent.ID_UNDEFPREDICATE, term.copy(),
	// nQueryHandle);
	// }

	/**
	 * Releases the resources used by JIProlog Developers should not call it
	 * directly.
	 */
