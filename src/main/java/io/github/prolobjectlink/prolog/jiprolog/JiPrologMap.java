 ...
 * @since 1.1
 */
public final class JiPrologMap extends AbstractCompounds implements PrologMap {

	private Map<PrologTerm, PrologTerm> map;

	JiPrologMap(PrologProvider provider, int size) {
		super(PrologTermType.MAP_TYPE, provider);
		map = new LinkedHashMap<PrologTerm, PrologTerm>(size);
	}

	JiPrologMap(PrologProvider provider, Map<? extends PrologTerm, ? extends PrologTerm> m) {
		this(provider);
		putAll(m);
	}

	JiPrologMap(PrologProvider provider) {
		this(provider, 16);
	}
	...
	public PrologTerm[] getArguments() {
		...
		for (int j = 0; j < args.length && i.hasNext(); j++) {
			...
			args[j] = new JiPrologEntry(p, e.getKey(), e.getValue());
		}
		return args;
	}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		JiPrologMap other = (JiPrologMap) obj;
		if (map == null) {
			if (other.map != null)
				return false;
	...
	public PrologTerm getTail() {
		JiPrologMap m = new JiPrologMap(provider, map);
	...
	private class PrologMapIterator extends AbstractIterator<PrologTerm> implements Iterator<PrologTerm> {
		...
		private PrologMapIterator() {
			...
			for (Iterator<Entry<PrologTerm, PrologTerm>> i = map.entrySet().iterator(); i.hasNext();) {
				Entry<PrologTerm, PrologTerm> e = i.next();
				PrologTerm t = new JiPrologEntry(provider, e.getKey(), e.getValue());
				set.add(t);
			}
			itr = set.iterator();
		}
